...
class WorksheetsController extends AppController {
	...
	private function handleUploads(){
		...
		$this->Attachment->actsAs['Uploader.Attachment']['file']['uploadDir'] = '/uploads/'.$this->Worksheet->id.'/';
		//$this->Uploader['uploadDir']='/uploads/files/'.$this->Worksheet->id.'/';	
		$attachmentCounter = 0;
		if(!empty($newAttachmentData)){
			foreach($newAttachmentData as $oneAttachment){
				...
				//debug($this->Attachment);exit;
//				$this->Uploader->move($this->Attachment['link'])	
			}
		}
	...
	function admin_submitWorksheetForm(){
		//debug($this->params['data']['Attachment']);exit;
